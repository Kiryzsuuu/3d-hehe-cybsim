...
- **World (hub 3D yang bisa dijelajahi)**: `/world`, karakter bisa jalan-jalan (WASD/panah) di peta 3D (Three.js + React Three Fiber) dengan kamera third-person yang mengikuti. Setiap fitur (Dashboard, Skenario, Room, Network Topology, Chat, Leaderboard, Profil, Tutorial) punya "stasiun" 3D sendiri di peta; mendekat memicu highlight + prompt "Tekan E", menekan E membawa masuk ke halaman fitur itu (portal, bukan reimplementasi ulang fitur di 3D). Diverifikasi dengan gerakan keyboard sungguhan lewat Playwright: jalan ke stasiun, prompt muncul, tekan E benar-benar navigasi.
