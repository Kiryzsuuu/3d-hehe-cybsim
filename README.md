...
- **Phase 3**: `/dashboard/scenarios` menampilkan daftar misi (3 skenario beginner), `/dashboard/scenarios/[slug]` untuk checklist objective, hint berjenjang, dan penyelesaian misi (skor tersimpan ke Progress). Dashboard menampilkan progress nyata dari MongoDB. Belum dibangun: auto-tracking objective dari aksi user (mis. command terminal atau aksi topologi otomatis mencentang objective), scenario editor/admin panel.
- **Belum dimulai**: Phase 4 (DVWA sandbox, blue-team scoring, CTF flags), Phase 5 (leaderboard, achievement, CI/CD).
