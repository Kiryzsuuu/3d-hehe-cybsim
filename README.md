...
- **Phase 4/5 (partial)**: sistem CTF flag (hash SHA-256 tersimpan di scenario data, verifikasi server-side dengan timing-safe compare, rate limit ketat 10/menit, hash tidak pernah diekspos ke client) dan `/leaderboard` global yang mengagregasi total skor per user. Belum dibangun: Docker sandbox manager per-user yang beneran jalan (skeleton sudah ada di `apps/api/src/services/sandbox`, belum diorkestrasi lewat UI), integrasi DVWA, firewall configuration simulator, vulnerability scanner simulation, achievement system, CI/CD pipeline.
